...
# ── Q-Learning ────────────────────────────────────────────────────────────────
def run_q_learning(env, Vs, n_iter, eps_start=1.0, eps_end=0.02, steps_scale=20):
    ...
    steps_per_iter = max(env.nS * steps_scale, 500)
    ...
    for it in range(n_iter):
        ...
        for _ in range(steps_per_iter):
            ...
            alpha = max(0.02, visit_counts[state, action] ** -0.6)
